...
def create_hyperparameter_combinations(API_KEY, training_set, validation_set):

    client = OpenAI(api_key=API_KEY)

    interviewOnly_train_id= client.files.create(file=open(training_set, "rb"), purpose="fine-tune").id
    interviewOnly_val_id= client.files.create(file=open(validation_set, "rb"), purpose="fine-tune").id
    ...
    batch_size = [32, 64]
    datasets = [[interviewOnly_train_id, interviewOnly_val_id], 
                [interviewAndQuestions_train_id, interviewAndQuestions_val_id]]
    hyperparameter_combinations = itertools.product(epochs, learning_rate_multiplier, batch_size, datasets)
    return hyperparameter_combinations

def fine_tune_models(API_KEY):
    hyperparameter_combinations = create_hyperparameter_combinations(API_KEY)
    client = OpenAI(api_key=API_KEY)
    ...
    for model_name in model_id:
        ...
        for sub_department in danish_radiograph_sentences:
            ...
            for idx, sentence in enumerate(phrases):
                ...
                reference_translation = ukranian_radiograph_sentences[sub_department][idx-1]
...
def generate_translations(API_KEY, model_id):
